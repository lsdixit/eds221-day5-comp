...
# create a time series of years to estimate population size
time_yr <- seq(from = 0, to = 8, by = 1)

# output matrix, column is each fish stage, and rows are time points
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# make first row initial population metrics
proj_year[1, ] <- fish_year0

# for loop to calculate population metrics over time. Starts at 2 because initial population is at proj_year[1, ] as indicated above
for (i in 2:length(time_yr)) { #start with year 1, year 0 is in position [1]
  proj_year[i,] <- fish_leslie %*% proj_year[i-1,]
}


# The rest is wrangling & visualization (run to check it out):
# updating column names for populaton stages
